...
@cli
def export(id_or_slug: str, source: str = "prod"):
    api = DatagouvfrAPI(url="http://www.data.gouv.fr", authenticated=False)
    ...
    with (open(path.joinpath("bouquet.csv"), "w") as bouquet_file,
        ...
        for factor_index, factor_payload in enumerate(datasets_properties, start=1):
            ...
            if factor.factor_availability == "url available":
                ...
                factor_dataset_id = factor_payload["id"]
                dataset_payload = api.get(f"/api/1/datasets/{factor_dataset_id}")
                factor.dataset_id = factor_dataset_id
